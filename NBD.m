...
x=flipud(x);
%flipud is a matlab function which flip a column vector - Up to Down
%fliplr is a matlab function which flip a row vector - Left to Right
y=reshape(y,number,1);
y=flipud(y);

%{
 Here I used flipud to flip the column vectors of x and y so the rest of
the NFD code can be used with minimum changes. Else entire matrix update
loop would be reqd to be changed.
%} 
